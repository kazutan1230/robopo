...
export const IpponBashi = ({ compeId, courseId, playerId, umpireId, point }: IpponBashiProps) => {
  ...
  const [botPosition, setBotPosition] = useState({ row: 9, col: 0 })
        ...
        <div className="flex flex-col">
          <div className="stats shadow">
            <div className="stat">
              ...
              {progress < IPPON_BASHI_SIZE - 1 ? (
                <p className="text-3xl font-bold text-orange-600">前進中</p>
              ) : (
                <p className="text-3xl font-bold text-orange-600">後進中</p>
              )}
            </div>
          </div>
...
const IpponBashiUI = (props: IpponBashiUIProps): JSX.Element => {
  ...
  return (
    <div className={"relative grid grid-cols-" + width + " grid-rows-" + length + " mx-auto"} style={ipponBashiStyle}>
      ...
      <>
        <Robot row={props.botposition.row} col={props.botposition.col} direction={direction} type={type} />
        <NextArrow
          row={props.botposition.row}
          col={props.botposition.col}
          direction={direction}
          nextMissionPair={props.progress < length - 1 ? nextMissionPairMF : nextMissionPairMB}
          duration={1.5}
          type={type}
        />
      </>
